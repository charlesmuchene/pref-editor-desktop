...
class NavigationTest {
    ...
    @Test
    fun `navigation always has home as start`() =
        runTest(dispatcher) {
            navigation.currentScreen.test {
                assertEquals(expected = DevicesScreen, actual = awaitItem())
            }
        }

    @Test
    fun `forward navigation appends screen to stack`() =
        runTest(dispatcher) {
            navigation.currentScreen.test {
                assertEquals(expected = DevicesScreen, actual = awaitItem())
    ...
    @Test
    fun `backwards navigation pops screens on stack exclusively`() =
        runTest(dispatcher) {
            navigation.navigate(AppsScreen(device))
    ...
    @Test
    fun `backwards navigation never pops the home screen`() =
        runTest(dispatcher) {
            navigation.navigate(AppsScreen(device))
            navigation.navigate(FilesScreen(app, device))
