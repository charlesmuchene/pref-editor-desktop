...
class PrefFileListDecoderTest {
    ...
    @Test
    fun `empty preferences when we are missing preference directory`() = runTest {
        val result = decoder.decode("ls: shared_prefs: No such file or directory")
        assertTrue(result is EmptyPrefs)
    }

    @Test
    fun `empty preferences when there are no files in the directory`() = runTest {
        val result = decoder.decode("")
        assertTrue(result is EmptyPrefs)
    }

    @Test
    fun `preference listing when preference directory is non empty`() = runTest {
        val content = """com.charlesmuchene.player_preferences.xml
            named.xml
        """.trimIndent()
    ...
    @Test
    fun `preference listing when app is not debuggable`() = runTest {
